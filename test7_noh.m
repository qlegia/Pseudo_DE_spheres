% In order to use this routine, you have to run "test7_U22" first 
% to get a good solution, then the good solution is saved to "int_goodU22.mat"
% -------------------------------------------------------------------------
...
%colorbar('vert');
%shading interp;
[FH,soln] = integral2(md27,f',27,13,0);
load int_goodU22;
err = abs(soln-u22);
% compute local error on a spherical cap C := {x . w>0.9}
% with w = (1/sqrt(2),0,1/sqrt(2))
max_cap_e = cap_err(51,err,[1/sqrt(2);0;1/sqrt(2)],0.9)
max_err = max(max(err)) 
err = err/max_err;
%figure;
%surf(Sx,Sy,Sz,err);

...
sol00 = cfh(1)/sqrt(4*pi)/Gre_lambda(1);
Isol_m = zeros(Lsol,Nr);
Isol = zeros(max_q,Nr);
Isol_m_noh = zeros(Lsol,Nr);
Isol_noh = zeros(max_q,Nr);
Isol00 = cfh(1)/sqrt(4*pi)/Int_lambda(1);
...
for q=1:max_q
   ...
   for ell=1:Lsol-1
      ...
      sol_m(ell+1,:) = (hsol(ell+1)/Gre_lambda(ell+1)).*sYell;
      sol_m_noh(ell+1,:) = sYell./Gre_lambda(ell+1);
      Isol_m(ell+1,:) = (hsol(ell+1)/Int_lambda(ell+1)).*sYell;
      Isol_m_noh(ell+1,:) = sYell./Int_lambda(ell+1);
      %sol_m(ell+1,:) = sYell;   
  end
  sol(q,1:Nr) = sum(sol_m); % sum over all ell
  sol_noh = sum(sol_m_noh);   
  Isol(q,1:Nr) = sum(Isol_m); % sum over all ell
  Isol_noh = sum(Isol_m_noh);   
  % add the constant sol00 = u_{0,0}
  sol(q,:) = sol(q,:) + hsol(1)*sol00;
  Isol(q,:) = Isol(q,:) + hsol(1)*Isol00; 
  % add the constant sol00 = u_{0,0} to the solution
  sol_noh = sol_noh + sol00;
  Isol_noh = Isol_noh + Isol00;
  ell
  toc
end

...
load md39.1600; % Driscoll-Healy quadrature X_W of 2^{14} points
X = md39(:,1:3);
% f = (X(:,1).*cos(X(:,3)) - X(:,3).*sin(X(:,3)) + 0.5*cos(X(:,3))).*exp(X(:,1));
f = non_smooth_f(X);
goodFH = Fhat(md39,f',39);
...
for with_h = 0:1
  ...
  goodU32 = zeros(4,Nr);
  goodU32(1,:) = cap_integral(goodFH,22,with_h,XR1);
  goodU32(2,:) = cap_integral(goodFH,22,with_h,XR2);
  goodU32(3,:) = cap_integral(goodFH,22,with_h,XR3);
  goodU32(4,:) = cap_integral(goodFH,22,with_h,XR4);
  % compute the approximation
  soln = zeros(4,Nr);
  soln(1,:) = cap_integral(FH,13,with_h,XR1);
  soln(2,:) = cap_integral(FH,13,with_h,XR2);
  soln(3,:) = cap_integral(FH,13,with_h,XR3);
  soln(4,:) = cap_integral(FH,13,with_h,XR4);
